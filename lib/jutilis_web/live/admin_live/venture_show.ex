defmodule JutilisWeb.AdminLive.VentureShow do
        ...
        <div class="mb-8">
          ...
          <div class="flex items-start justify-between">
            ...
            <div class="flex items-center gap-2">
              ...
              <%= if @venture.url do %>
                <a href={@venture.url} target="_blank" rel="noopener noreferrer" class="btn btn-primary btn-sm">
                  <svg class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="mb-8">
          ...
          <%= if Enum.empty?(@venture.links) do %>
            <div class="text-center py-16 rounded-2xl border-2 border-dashed border-base-300">
              <svg class="h-12 w-12 mx-auto text-base-content/30 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
              </svg>
              <p class="text-base-content/60 font-semibold">No quick links yet</p>
              <p class="text-sm text-base-content/40 mt-1">Add links to dev environments, hosting, banking, and more</p>
                        ...
                        <div class="flex items-center gap-4 flex-1 min-w-0">
                          <div class="flex-shrink-0">
                            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-base-200 group-hover:bg-base-300 transition-colors">
                              <.category_icon category={link.category} class="h-5 w-5 text-base-content/60" />
                            </div>
                          </div>
                          <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                              <a href={link.url} target="_blank" rel="noopener noreferrer" class="font-semibold text-base-content hover:text-primary transition-colors truncate">
                                {link.name}
                              </a>
                              <svg class="h-4 w-4 text-base-content/40 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                              </svg>
                            </div>
                            ...
                            <% end %>
                            <p class="text-xs text-base-content/40 truncate">{URI.parse(link.url).host}</p>
                          </div>
                        </div>
                        <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button phx-click="edit_link" phx-value-id={link.id} class="btn btn-ghost btn-xs">
                            Edit
                          </button>
                          <button phx-click="delete_link" phx-value-id={link.id} data-confirm="Delete this link?" class="btn btn-ghost btn-xs text-error">
                            Delete
                          </button>
                        </div>
                      </div>
          ...
          <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <%= if @venture.url do %>
              <div class="sm:col-span-2">
                ...
                <dd class="mt-1">
                  <a href={@venture.url} target="_blank" rel="noopener noreferrer" class="text-primary hover:underline">
                    {@venture.url}
                  </a>
                </dd>
              </div>
            ...
            <div class="form-control">
              ...
              >
                <%= for {label, value} <- VentureLink.category_labels() do %>
                  <option value={value} selected={@link_form[:category].value == value}>{label}</option>
                <% end %>
              </select>
            </div>
  ...
  def handle_event("show_add_link", _params, socket) do
    changeset = Ventures.change_venture_link(socket.assigns.current_scope, %VentureLink{
  ...
  def handle_event("save_link", %{"venture_link" => link_params}, socket) do
    ...
    result =
      if socket.assigns.editing_link do
        Ventures.update_venture_link(socket.assigns.current_scope, socket.assigns.editing_link, link_params)
      else
    ...
    case result do
      {:ok, _link} ->
        venture = Ventures.get_venture_with_links!(socket.assigns.current_scope, socket.assigns.venture.id)
        links_by_category = Ventures.get_links_by_category(venture)
        ...
        {:noreply,
         ...
         |> assign(:editing_link, nil)
         |> assign(:link_form, nil)
         |> put_flash(:info, if(socket.assigns.editing_link, do: "Link updated", else: "Link added"))}
