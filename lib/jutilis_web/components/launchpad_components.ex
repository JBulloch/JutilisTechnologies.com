defmodule JutilisWeb.LaunchpadComponents do
  ...
  def step_card(assigns) do
    ...
    <div class="rounded-2xl border-2 border-base-300 bg-base-100 overflow-hidden mb-4">
      ...
      <div :if={@expanded && length(@category_links) > 0} class="p-4 border-b border-base-200 bg-success/5">
  ...
  def tool_item(assigns) do
    ...
    <div class={[
      ...
      if(@tool.is_featured, do: "bg-primary/5 border border-primary/20", else: "bg-base-200/50 hover:bg-base-200")
    ]}>
      ...
      <div class="flex-1 min-w-0">
        ...
        <%= if @tool.factoid do %>
          <p class="text-xs text-base-content/50 flex items-start gap-1">
            <svg class="h-4 w-4 flex-shrink-0 text-primary/60" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div>
          ...
          <div class="border-l-2 border-base-300 ml-5 pl-6">
            <%= for category <- categories do %>
              <.step_card category={category} step_number={category.display_order} user_links={@user_links} />
            <% end %>
