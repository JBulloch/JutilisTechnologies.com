defmodule JutilisWeb.UserAuth do
  ...
  @remember_me_cookie "_jutilis_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      JutilisWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def require_admin(conn, _opts) do
    if conn.assigns.current_scope && conn.assigns.current_scope.user &&
         conn.assigns.current_scope.user.admin_flag do
      conn
    else
      conn
  ...
  @doc """
  Used for routes that require the user to be authenticated in LiveView.

  ## `on_mount` arguments

    * `:mount_current_scope` - Assigns current_scope to the socket
    * `:ensure_authenticated` - Authenticates user and assigns current_scope
    * `:redirect_if_authenticated` - Redirects if user is already authenticated
    * `:ensure_admin` - Ensures the current user is an admin

  ## Examples

      defmodule JutilisWeb.PageLive do
        use JutilisWeb, :live_view

        on_mount {JutilisWeb.UserAuth, :mount_current_scope}
      end
  """
  ...
  def on_mount(:ensure_admin, _params, session, socket) do
    ...
    if socket.assigns.current_scope && socket.assigns.current_scope.user &&
         socket.assigns.current_scope.user.admin_flag do
      {:cont, socket}
    else
      socket =
        socket
